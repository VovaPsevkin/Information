__author__ = 'Vova Psevkin'
"""
Submitted by: Vladimir Psevkin
i.d: 318013356
date: 25.03.2020
"""
...
def test_reed_solomon_approach(original_message_code, message):
    print("\nTest the integrity of the software")
    # sorting both the lists
    original_message_code.sort()
    message.sort()

    # using == to check if
    # lists are equal
    if original_message_code == message:
...
def main(message):
    ...
    tSize = len(message)
    print(f"Sentence to encode and test 'Reed–Solomon code'\n\"{message}\"\n"
          f"Length of message: {tSize}", end='\n' * 2)
    # encode the message
    tCode = fooRS.RSEncode(message, tSize)
    original_message_code = deepcopy(tCode)
    print(f"Message codeword: \n{tCode}")
    print("\r\r")

    # introduce errors/erasures
    print("Introduce errors/erasures")
    num1, num2, num3 = random.randint(0, 100), random.randint(0, 100), random.randint(0, 100)
    ind1, ind2, ind3 = tSize // 3, tSize // 2, tSize - 20
    print(f"Error one index: {ind1}, value error: {num1}")
    print(f"Error two index: {ind2}, value error: {num2}")
    print(f"Error three index: {ind3}, value error: {num3}")
    print("\r\r")

    tCode[ind1], tCode[ind2], tCode[ind3] = num1, num2, num3
    print(f"Message codeword (with three errors/erasures): \n{tCode}")
    print("\r\r")

    # decode the message
    message = fooRS.RSDecode(tCode, tSize)
    print("\nDecoded message: ", message)
    if test_reed_solomon_approach(original_message_code, message):
        print("The software works properly")
    else:
        raise ValueError("Bug in software")


if __name__ == '__main__':
    try:
        parser = argparse.ArgumentParser(description='python script.py \
        --message=For example, a coding statement and error correction')
        parser.add_argument('--message',
                            type=str,
                            default=f"We were all a little drunk with spring, "
                                    f"like the fat bees reeling from flower to flower, "
                                    f"and a strange insurrectionary current ran among us",
                            nargs='?',
                            help='message')
